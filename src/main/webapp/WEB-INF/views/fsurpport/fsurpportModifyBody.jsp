...
<form action="${pageContext.request.contextPath}/fsurpport/modifyFarmdiary" method="post" enctype="multipart/form-data">
	...
	<input type="hidden" name="writer" value="${S_USER.user_id }" required="required">
	<input type="hidden" name="f_diary_no" value="${farmdiaryList.f_diary_no }" required="required">
	...
	<div class="form-group col-md-6">
		...
		<select name="w_step_code">
			...
			<c:forEach items="${workstepsList }" var="workstepsList">
				<option value="${workstepsList.code_no }" <c:if test="${workstepsList.code_nm == farmdiaryList.w_step_code}">selected</c:if>>${workstepsList.code_nm }</option>
			</c:forEach>
		</select>
	</div>
	...
	<div class="form-group">
		...
		<div class="mailbox-attachments clearfix" style="text-align: center; width: 100%;">
			<div class="mailbox-attachment-icon has-img" id="pictureView" style="border: 1px solid green; height: 200px; width: 140px; margin: 0 auto;">
				<img id="pictureViewImg" style="width: 100%; height: 100%;" src="${pageContext.request.contextPath}/fsurpport/filePath?file_nm=${farmdiaryList.file_nm }" />
			</div>
